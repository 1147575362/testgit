...
#include <applicationTask.h>  // modify 2023.11.20
...
uint8_t LNCI_HandleNodeCfgIdentifyRequest(const DiagReqInfo_t *const diagReq,DiagRspInfo_t *const diagRsp)
{
  ...
  AssignID_t *Assign = (AssignID_t *)((void *)diagReq->payload);
  ...
  { /* Save Configuration */
      ...
      */
     // 2024.01.16  modify 打开颜色校准参数保存功能（仅83211）
      diagRsp->packLength = 0U;
      response = TRUE;
      TM_PostTask(TASK_ID_PDS);  //start persistance data storage (PDS )task 
  }
  else if (diagReq->sid == LIN_SID_SNPD && diagReq->packLength == sizeof(SNPD_t))
  { /* 0xB5 for auto addressing */
  ...
  { /* Read by Identifier */
      ...
      if ((readById->supplierID == LINS_SUPPLIER_ID || readById->supplierID == LIN_SUPPLIER_ID_WILDCARD)&&
          ...
          */
          ...
          {
              ReadHWSWVersion_t *rsp = (ReadHWSWVersion_t *)((void *)diagRsp->payload);
              rsp->SW = SW_Version;
              rsp->HW = HW_Version;
              diagRsp->packLength = (uint16_t)sizeof(ReadHWSWVersion_t);
              response = TRUE;
          }else{
              /*
