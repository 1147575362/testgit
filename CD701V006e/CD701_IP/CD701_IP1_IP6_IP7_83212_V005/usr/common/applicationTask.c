...
LedParamTemp_t LedParamTemp[24];      // max support number
uint8_t LedIndexTemp;

static uint8_t LED_State = 0U;
static uint8_t SlaveSerialNumber = 1;
const uint16_t nad2lednum_lut[] = {0,4,0,0,0,0,4,4,0,0,0,0,0,0,0,0};
static uint16_t SlaveLEDNum = 1;
...
void LedOnPorcess(uint8_t ledIndex, uint8_t ColorIndex, uint16_t Brightness, uint16_t transitionTime)
{
  ...
  CLM_SetXYY(ledIndex, temperature, (uint16_t)((rgb2xyyTable[ColorIndex].Cx + x) * COLOR_MAX), (uint16_t)((rgb2xyyTable[ColorIndex].Cy + y) * COLOR_MAX), Brightness, transitionTime);
}
...
void APPL_DataHandle()
{
  ...
  switch (SlaveSerialNumber)
  {
  case 1:
    SlaveLed = BCM_FrameNum_2->matrix.slave_1;
    break;
  case 2:
    SlaveLed = BCM_FrameNum_2->matrix.slave_2;
    break;
  case 3:
    SlaveLed = BCM_FrameNum_3->matrix.slave_3_H << 12 | BCM_FrameNum_2->matrix.slave_3_L;
    break;
  case 4:
    SlaveLed = BCM_FrameNum_3->matrix.slave_4;
    break;
  case 5:
    SlaveLed = BCM_FrameNum_3->matrix.slave_5;
    break;
  case 6:
    SlaveLed = BCM_FrameNum_3->matrix.slave_6;
    break;
  case 7:
    SlaveLed = BCM_FrameNum_3->matrix.slave_7;
    break;
  default:
    break;
  }
  ...
  else if (BCM_FrameNum_1->matrix.mode == 0x00) // if1
  {
...
static void Led_InitParam(void)
{
  ...
  SlaveSerialNumber = ls_read_nad();
  if(SlaveSerialNumber > 0xF){
    SlaveSerialNumber = 0x1;
  }
  SlaveLEDNum = nad2lednum_lut[SlaveSerialNumber];
