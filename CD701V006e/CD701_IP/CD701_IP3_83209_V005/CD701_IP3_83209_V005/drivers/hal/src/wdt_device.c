...
{
    uint8_t rev = (uint8_t)SYSCTRLA_SFRS->REV;
    uint8_t timeInterval = (uint8_t)interval;
    if (rev != (uint8_t)'0'){
        timeInterval = (timeInterval > 0U)? (timeInterval - 1U): timeInterval;
    }
    WDTA_SFRS->CTRL.TIMEOUTSEL = (uint8_t)timeInterval;
    CRGA_SFRS->WDTACTION.WDTBARKTHRESH = 5;
