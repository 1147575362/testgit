/**
 * @copyright 2019 indie Semiconductor
 *
 * This file is proprietary to indie Semiconductor.
 * All rights reserved. Reproduction or distribution, in whole
 * or in part, is forbidden except by express written permission
 * of indie Semiconductor.
 *
 * @file wdta_sfr.h
 */

#ifndef WDTA_SFR_H__
#define WDTA_SFR_H__

#include <stdint.h>

typedef union{
  struct {
    uint8_t  UPDATE                   :  1; /* only valid for Version C1 and later*/
    uint8_t  RUNNING                  :  1; /*!< Running status */
    uint8_t                           :  6; /*   (reserved) */
    uint8_t  TIMEOUTSEL               :  3; /*!< Timeout select */
    uint8_t  WINOPENENA               :  1; /*!< Window Mode Enable */
    uint8_t  WINOPENSEL               :  2; /*!< Window Mode open select */
    uint8_t  WINOPENFLAG              :  1; /*!< Window open flag */
    uint8_t                           :  1; /*   (reserved) */
    uint16_t                          : 16; /*   (reserved) */
  };
  uint32_t WORD;
}WDTA_CTRL_t;

typedef union{
  struct {
    uint8_t  STOP                     :  8;
    uint8_t                           :  8; /*   (reserved) */
    uint8_t                           :  8; /*   (reserved) */
    uint8_t                           :  7; /*   (reserved) */
    uint8_t  STOP_LOCK                :  1;
  };
  uint32_t WORD;
}WDTA_STOP_t;

/* Only Valid for Version C1 and later*/
typedef union{
  struct {
    uint8_t  TIMEOUT_EN                 :  1; /*  WDTA timeout Interrupt Enable. */
    uint8_t  WINDOW_OPEN_EN             :  1; /*  Window Open Interrupt Enable. */
    uint8_t                             :  6;
    uint8_t  TIMEOUT_CLEAR              :  1; /*  WDTA timeout Interrupt Clear. */
    uint8_t  WINDOW_OPEN_CLEAR          :  1; /*  Window Open Interrupt Clear. */
    uint8_t                             :  6;
    uint8_t  TIMEOUT_STATUS             :  1; /*  WDTA timeout Status */
    uint8_t  WINDOW_OPEN_STATUS         :  1; /*  Window Open Status */
    uint8_t                             :  6;
    uint8_t  TIMEOUT_INT_STATUS         :  1; /*  WDTA timeout Interrupt Status. */
    uint8_t  WINDOW_OPEN_INT_STATUS     :  1; /*  Window Open Interrupt Status. */
    uint8_t                             :  6;
  };
  uint32_t WORD;
}WDTA_INTERRUPT_t;

/**
 * @brief A structure to represent Special Function Registers for WDTA.
 */
typedef struct {
  WDTA_CTRL_t   CTRL;           /* +0x000 */
  WDTA_STOP_t   STOP;           /* +0x004 */
  uint32_t      CLEAR;          /* +0x008 */
  uint32_t      CNTVAL;         /*<! Counter value +0x00C */
  WDTA_INTERRUPT_t INTERRUPT;   /* Only Valid for Version C1 and later*/
} WDTA_SFRS_t;

/**
 * @brief The starting address of WDTA SFRS.
 */
#define WDTA_REG_CTRL           (*(__IO WDTA_CTRL_t *)(0x50010300))
#define WDTA_REG_STOP           (*(__IO WDTA_STOP_t *)(0x50010304))
#define WDTA_REG_CLEAR          (*(__IO    uint32_t *)(0x50010308))
#define WDTA_REG_CNTVAL         (*(__IO    uint32_t *)(0x5001030C))
#define WDTA_REG_INTERRUPT      (*(__IO    uint32_t *)(0x50010310)) /* Only Valid for Version C1 and later*/

#define WDTA_SFRS ((__IO WDTA_SFRS_t *)0x50010300)

#endif /* end of __WDTA_SFR_H__ section */


