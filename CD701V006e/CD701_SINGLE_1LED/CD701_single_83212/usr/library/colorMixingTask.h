...
typedef struct{
  uint8_t  device;              /* 1: means realplum(IND83205) , 2: rugby(IND83204)  3: realplumPro(IND83209),4: rugbyPro(IND83211),5: realplumLite(IND83212)*/
  uint8_t  ledDriverType;       /* 0: meta RGB LED  others: multi- leds with time division,0xFF:Mixed */
  uint16_t version;             /* main Version 0x1000 means V1.0.0.0       */
}LibVersionInfo_t;
...
/*
 *  ColorMixing Intensity Degradation enable, there is about 3% Intensity Degradation by default
 *  @param [in] enable: 0U: disable degradation, others: enable degradation
 *  @return 0
 */
uint8_t CLM_SetIntensityDegradation(uint8_t enable);

/*
 *  Get ColorMixing Intensity Degradation enable state.
 *  @param [in] none
...
/*
 *  Set xy absolute physical intensity output  for example: P(x,y) = (0.33333,0.33333) x = 0.33333*65536 = 21845, y = 0.33333*65536 = 21845  AbsY: lumen or mcd
 *  @param [in] temperature: -40-100 in 1C
 *  @param [in] x: 0-65535
 *  @param [in] y: 0-65535
 *  @param [in] intensity: 0-65535, lumen or mcd, if intensity value is higher than maximum intensity of the target color, it would display maximum intensity of the target color.
 *  @param [in] transitionTime: 0-65535ms
 *  @return 0
 */
uint8_t CLM_SetXYAbsY(LedNum_t ledIndex,ColorTemperature_t temperature, uint16_t x, uint16_t y, uint16_t intensity, uint16_t transitionTime);
...
/*
 *  Set uv absolute phy level output,level: lumen or mcd
 *  @param [in] temperature: -40-100 in 1C
 *  @param [in] u: 0-1023
 *  @param [in] v: 0-1023
 *  @param [in] level: 0-65535, lumen or mcd, if intensity value is higher than maximum intensity of the target color, it would display maximum intensity of the target color.
 *  @param [in] transitionTime: 0-65535ms
 *  @return 0
 */
uint8_t CLM_SetAbsLUV(LedNum_t ledIndex,ColorTemperature_t temperature, uint16_t u, uint16_t v, uint16_t level, uint16_t transitionTime);
...
/*
 *  Set sRGB absolute phy intensity output lumen or mcd, 
 *  @param [in] temperature: -40-100 in 1C
 *  @param [in] red: 0-255
 *  @param [in] green: 0-255
 *  @param [in] blue: 0-255
 ...
 *  @return 0
 */
uint8_t CLM_SetSRGBAbsL(LedNum_t ledIndex,ColorTemperature_t temperature, uint8_t red, uint8_t green, uint8_t blue,uint16_t intensity, uint16_t transitionTime);
 ...
 *  @param [in] temperatureR,G,B: current temperature -40-100 in 1C
