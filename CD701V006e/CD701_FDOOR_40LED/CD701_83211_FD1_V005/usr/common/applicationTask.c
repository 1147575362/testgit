...
void delayTimerExpired(SoftTimer_t *timer)
{
  // TM_PostTask(TASK_ID_APPL);
  APPL_DataHandle();
}
static SoftTimer_t delayTimer = {
    .mode = TIMER_ONE_SHOT_MODE,
    .interval = 8U,
    .handler = delayTimerExpired};
...
void APPL_TaskHandler(void)
{
  ...
  case TASK_STATE_INIT:
    CRC32_Init();
    Led_InitParam();
    APPL_RegisterDataProtocol(APPL_HandleBcmControlCommands);
...
void APPL_DataHandle()
{
  ...
  else if (BCM_FrameNum_1->matrix.mode == 0x00)
  {
    ...
    LedFlag = 1;
    SlaveLed = BCM_FrameNum_3->matrix.slave_1;
    for (uint8_t LedIndex = 0; LedIndex < SlaveLEDNum; LedIndex++, LedFlag *= 2)
    {
#ifdef left_front_door
      LedIndexTemp = SlaveLEDNum - LedIndex - 1;
  ...
  if ((color->nodeAddress == ls_read_nad()) || (color->nodeAddress == LIN_NAD_WILDCARD))
  {
    ...
    if (channel < LED_NUM)
    {
      ...
      case CMD_COLOR_CTRL_XYY:
        //2024.02.02 modify
        (void)CLM_SetXYY(ledIndex, MES_GetLedTemperature(ledIndex) /*temperature*/, color->ColorXY.x, color->ColorXY.y, (uint16_t)color->ColorXY.Y * 10U, (uint16_t)color->ColorXY.transitionTime * 100U);
        break;
