...
uint8_t VoiceLamp(uint8_t data[], uint8_t size, uint8_t offset, uint8_t max_len_left, uint8_t max_len_right)
{
	if (data == NULL)
		return 0;
	if (max_len_left > offset || max_len_right > size * 8 - offset)
		return 0;
	memset(data, 0x00, size);
	if(max_len_left == 0 && max_len_right == 0)
		return 0;

	// 处理左边
	for (uint8_t i = offset - 1; i > offset - max_len_left - 1; i--) 
	{
		data[i / 8] |= 0x80 >> (i % 8);
	}

	// 处理右边
	for (uint8_t i = offset - 1; i < offset + max_len_right - 1; i++)
	{
		data[i / 8] |= 0x80 >> (i % 8);
	}
}
